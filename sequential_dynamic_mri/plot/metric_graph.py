...
import numpy as np
from ..utils.score_metrics import mse, psnr, mssim
...
def calc_mssim(recon_img, gndtruth):
    ...
    for i in range(num_samples):
        for j in range(nt):
            mssim_error[i,j] = mssim(recon_img[i,:,:,j],gndtruth[i,:,:,j])
...
def create_plot(error_dict, metric_type, save_loc):
    for recon_type, val in error_dict.items():
        ...
        plot.plot(range(1,len(time_mean)+1),time_mean, label=recon_type)
